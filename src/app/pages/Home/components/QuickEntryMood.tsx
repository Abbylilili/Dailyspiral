...
const QuickEntryMood: FC<QuickEntryMoodProps> = ({ initialMood, date, onRefresh }) => {
  ...
  const handleSave = async () => {
    await saveMood({ id: date, date, mood });
      ...
      <CardContent className="space-y-4 flex flex-col flex-1">
        ...
        <div className="space-y-2 flex-1">
          <style>
            {`
              .neon-thumb-home {
                border: 2px solid ${theme === 'ocean' ? '#fff' : '#e5e7eb'} !important;
                transition: all 0.2s ease;
              }
              .neon-thumb-home:hover { transform: scale(1.5); }
            `}
          </style>
          <Slider value={[mood]} onValueChange={(v) => setMood(v[0])} min={1} max={10} step={1} thumbClassName="neon-thumb-home" />
